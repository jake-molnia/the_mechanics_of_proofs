...
example {n : ℤ} (hn : n ≥ 5) : n ^ 2 > 2 * n + 11 :=
  ...
  _ = 2 * n + 3 * n := by ring
  _ ≥ 2 * n + 3 * 5 := by rel [hn]
  _ = 2 * n + 11 + 4 := by ring -- we write 11 + 4 to use extra tactic
  _ > 2 * n + 11 := by extra
...
example {x : ℤ} (hx : x ≥ 9) : x ^ 3 - 8 * x ^ 2 + 2 * x ≥ 3 :=
  ...
  _ ≥ x^2 * 9 - 8 * x^2 + 2 * x := by rel [hx, hx]
  _ = x * x + 2 * x := by ring
  _ ≥ 9 * x + 2 * x := by rel [hx]
  _ = 11 * x := by ring
  _ ≥ 11 * 9 := by rel [hx]
...
example {n : ℤ} (hn : n ≥ 10) : n ^ 4 - 2 * n ^ 2 > 3 * n ^ 3 :=
  calc
  n^4 - 2 * n^2 = n^3 * n - 2 * n^2 := by ring
  _ ≥ n^3 * 10 - 2 * n^2 := by rel [hn]
  _ = n^3 * 3 + (n^2 * n * 7 - 2 * n^2) := by ring
  _ ≥ n^3 * 3 + (n^2 * 10 * 7 - 2 * n^2) := by rel [hn]
